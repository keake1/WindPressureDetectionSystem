...
void spl06_pressure_rate_config(u8 background_rate,u8 oversamply)
{
		...
		{
        data1 = spl0601_read(HW_ADR, CFG_REG);//读取原寄存器值
        data1 |= 0X04;//P-SHIFT位置1
				spl0601_write(HW_ADR, CFG_REG, data1);//重新写回寄存器					
		}		
		switch(oversamply)
    {
		...
		spl0601_write(HW_ADR, PRS_CFG_REG, data0);//写入配置
}
...
void spl06_temperature_rate_config(u8 background_rate,u8 oversamply,u8 ext)
{
		...
		switch(oversamply)
    {
		...
		spl0601_write(HW_ADR, TMP_CFG_REG, data0);//写入配置
}
/***********************************************************************
 * 传感器测量模式配置与状态读取
 * @param[in] 
...
void spl06_set_interrupt(u8 interrupt1,u8 type)//设置中断使能
{
	u8 data0;
	data0 = spl0601_read(HW_ADR, CFG_REG);
	if(type!=ENABLE )
		data0 &= ~interrupt1;
	else
		data0 |= interrupt1;	
	spl0601_write(HW_ADR, CFG_REG,data0);
}
