...
#   ./tools/flash.sh -f hex文件/main.hex      # 指定固件文件
#   ./tools/flash.sh -p /dev/ttyUSB0 -f hex文件/main.hex
#
# 注意:
#   - STC 下载需手动断电重上电，在提示 "Power cycle the MCU" 时操作。
#   - 此芯片 IRC 振荡器未校准，需用 -t 参数指定频率（默认 11059 = 11.059MHz）。
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -p) PORT="$2";  shift 2 ;;
        -f) FILE="$2";  shift 2 ;;
        -t) TRIM_KHZ="$2"; shift 2 ;;
        *)  echo "用法: $0 [-p 串口] [-f 固件文件] [-t 频率kHz]" >&2; exit 1 ;;
    esac
done
...
if [ -z "$FILE" ] || [ ! -f "$FILE" ]; then
    echo "错误: 未找到固件文件 (hex文件/main.hex)，请先在 Windows 上用 Keil 编译。" >&2
    exit 1
fi
echo "固件: $FILE"
...
echo "      断开目标板电源后重新上电。"
echo ""
python -m stcgal -p "$PORT" -b 115200 -t "$TRIM_KHZ" "$FILE"
